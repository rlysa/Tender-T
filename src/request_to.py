...
def get_cards(category_words):
    ...
    category_lots, cards, card_lots, lots, urls = {}, {}, {}, {}, []
    start_date, end_date = get_date()
    url = get_url(start_date, end_date)

    for category in category_words:
        category_lots[category] = []
        print(f'{datetime.now()}: Поиск карточек: {len(category_lots)}/{len(category_words)}')
        for word in category_words[category]:
            ...
            for block in blocks:
                ...
                if number in cards:
                    for article in card_lots[number]:
                        category_lots[category].append(article)
                    continue
    ...
    return [category_lots, cards, card_lots, lots, urls]
