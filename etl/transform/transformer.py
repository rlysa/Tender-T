...
from config import COST_OUTPUT_TOKENS, COST_INPUT_TOKENS


def transform_cards(script, cards, card_lots, lots):
    ...
    for category in category_products:
        ...
        try:
            products_text = '\n'.join([f'{product}: {products[product][0]}; {products[product][1]}' for product in
                                       category_products[category]])

            lots_text = '\n'.join([f'{lot}: {lots[lot]['name']} ({lots[lot].get('description', '')})' for lot in
                                   category_true_lots[category]])
    ...
    return [result, cost, looked_cards + not_looked_cards]